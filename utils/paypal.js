...
module.exports = async function (amount, email) {
    ...
    if (!accessToken || Date.now() > expirationDate) {
        accessToken = (await axios.post('https://api-m.sandbox.paypal.com/v1/oauth2/token', 'grant_type=client_credentials', {
            headers: {
    ...
    //Send money
    try {
        let payout = await axios.post('https://api-m.sandbox.paypal.com/v1/payments/payouts', {
            "sender_batch_header": {
