...
async function checkFinished() {
    ...
    setInterval(async function () {
        //Calculate ffa matches
        ffaEnd()
        // Only process quests once every 3 minutes
        time++
        if (!time >= 3) return time = 0
        ...
        for (const element of userNeedingUpdate) {
            ...
            // Not with a "for", "for" are less readable and in this case it doesn't save any line of code
            ...
            let stats = await callBrawlhallaApi(element.brawlhallaId, "all", false)
            ...
            for (let i = 0; i < 2; i++) {
                ...
                element.solo[currentQuestArrayName].forEach((elemen, index) => {
                    ...
                    let actualQuestData = actualsDaily([actualQuest], stats,)
                    // If the quest is finished then make it collectable and add the notification
                    if ((actualQuestData.actual - elemen.actual) + elemen.progress >= elemen.goal) {
                        if (!newNotification)
                            newNotification = {
                                id: 1,
