...
async function calculateRewards(element, isForced) {
    const filteredPlayerList = element.players.filter(p => {
        if (p.updateCount <= 0 && p.joinDate + 1800 * 1000 < Date.now() && p.gamesPlayed < 7) {
            return true
        } else if (p.updateCount === 1 && p.joinDate + 3600 * 1000 < Date.now() && p.gamesPlayed < 7) {
            return true
        }
    });
    let updated = false
    for (const player of filteredPlayerList) {
        ...
        if (games !== player.totalGamesPlayed) {
            ...
            element.players[element.players.findIndex(e => e.steamId === player.steamId)].wins = gp <= 7 ? wins : Math.round((wins) / (gp / 7))
        }
    }
    if (updated) {
        setTimeout(async () => {
