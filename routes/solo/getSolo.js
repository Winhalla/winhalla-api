...
router.get('/solo', async function (req, res) {
    ...
    try {
        ...
        if (player.waitingNewQuestsDaily === true || player.waitingNewQuestsWeekly === true) {

            // Get random quests for needed types
            let questsWithoutUserData = random(player.waitingNewQuestsDaily, player.waitingNewQuestsWeekly, player.solo.lastQuests)
        ...
        // If player already got quests
            ...
            if(player?.lastRefresh?.quests && player?.lastRefresh?.quests + 600 * 1000 > Date.now()) return res.send({solo:player.solo})
