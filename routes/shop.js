...
router.post('/buy/:id', async (req, res) => {
    try {
        ...
        if (!shop1.platforms.some(name => user1.brawlhallaAccounts.some(platform => platform.platformId === name) || name === "all") && !user1.declaredSteamId)
        ...
        if (shop1.type === "paypal" && req.query.number < 10 && req.query.number >= 1) {
        ...
        await commands.create({
            steamId: playerPlatform === "steam" ? req.user.id : user1.declaredSteamId,
            product: shop1.name,
            date: Date.now(),
            source: "Shop",
        ...
        let htmlStr = renderFx(shop1.type === "paypal" ? "paypalShop" : "shop", {object: shop1.name})
        // Send mail
        process.mailer.sendMail({
            html: htmlStr,
            text: htmlStr.replace(/(<([^>]+)>)/ig, ''), // For some reason the text value is required too (dont' remove the mail won't send)
            to: req.query.email,
            from: "Winhalla <postmaster@winhalla.app>",
            subject: 'Your command in Winhalla',
        });
